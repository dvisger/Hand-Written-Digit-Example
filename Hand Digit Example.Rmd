...
I have modified it to run in R as the original example was for octave(matlab). The data is included for reference
...
In this example we are using a pre trained network, which are the weights that have been optimized for this problem given this network.
...
To be able to do matrix matrix multiplication we need to add a constant for the bias in the input data:
```{r}
...
The predictions are not perfect, for example:
```{r}
show_image(4991)
predicted_y[4991]
y[4991]
